...
- 数据快照
  ...
  snapCount 参数配置每次数据快照之间的事务操作次数，即 ZooKeeper 会在 snapCount 次事务日志记录后进行一个数据快照

  数据快照的过程：

  1. 确定是否进行数据快照

     过半随机策略，分散快照时机，避免集群并发压力
...
ZooKeeper 启动期间，首先进行数据初始化工作，讲存储在硬盘上的数据加载到内存数据库中
...
处理快照文件
  ...
  4. 获取最新的 100 个快照文件
     ...
     每一个快照文件都是全量数据，所以最晚的快照文件包含最新的全量数据

  5. 解析快照文件

     从最晚生成的快照文件开始解析，解析失败则前推一个解析
